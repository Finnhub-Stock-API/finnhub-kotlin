/**
 * Finnhub API
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package io.finnhub.api.models


import com.squareup.moshi.Json
import java.io.Serializable

/**
 * 
 *
 * @param ebitAvg Average EBIT estimates including Finnhub's proprietary estimates.
 * @param ebitHigh Highest estimate.
 * @param ebitLow Lowest estimate.
 * @param numberAnalysts Number of Analysts.
 * @param period Period.
 * @param year Fiscal year.
 * @param quarter Fiscal quarter.
 */

data class EbitEstimatesInfo (

    /* Average EBIT estimates including Finnhub's proprietary estimates. */
    @Json(name = "ebitAvg")
    val ebitAvg: kotlin.Float? = null,

    /* Highest estimate. */
    @Json(name = "ebitHigh")
    val ebitHigh: kotlin.Float? = null,

    /* Lowest estimate. */
    @Json(name = "ebitLow")
    val ebitLow: kotlin.Float? = null,

    /* Number of Analysts. */
    @Json(name = "numberAnalysts")
    val numberAnalysts: kotlin.Long? = null,

    /* Period. */
    @Json(name = "period")
    val period: kotlin.String? = null,

    /* Fiscal year. */
    @Json(name = "year")
    val year: kotlin.Long? = null,

    /* Fiscal quarter. */
    @Json(name = "quarter")
    val quarter: kotlin.Long? = null

) : Serializable {
    companion object {
        private const val serialVersionUID: Long = 123
    }

}

